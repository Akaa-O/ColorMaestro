...
public class FrenzyBlock {
        ...
        myActivity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if(blockTouched == true) {  //block was touched
                    ...
                    final int blockSpeedTemp = ClassicActivity.blockSpeed;
                    ClassicActivity.blockSpeed = ClassicActivity.blockSpeedLimit;
                    ...
                    TimerTask frenzyEndTask = new TimerTask() {
                        ...
                        public void run() {
                            ...
                            ClassicActivity.blockSpeed = blockSpeedTemp; //Note:This can undo the blockspeed decay if player was still in frenzy when the score reached
    ...
    public void handleBlockTouch(){ //handles the touch event accordingly
        ...
        if(ClassicActivity.classicPrefs.getBoolean(GlobalVariables.soundSwitch,true)) {
            ClassicActivity.sound.playFrenzySound();
        }
        myActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
        ...
        final TimerTask blockGenerationTask = new TimerTask() {
            ...
            public void run() {
                ...
                while (positionX >= (GlobalVariables.queryInt("previouspos")-GlobalVariables.queryInt("blockwidth")) &&
                    ...
                    positionX = rand.nextInt(ClassicActivity.sizeX);                                      //offset of 75 used(half of block width) to avoid overlapping of blocks
                }
                ...
                //creates block
                if(GlobalVariables.isGamePaused == false){
                    ClassicActivity.generateBlock(myActivity,GlobalVariables.classicGameContext,positionX);
                }
