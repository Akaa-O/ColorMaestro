...
//This block gives destroys all blocks of the same screen color, thereby giving you the points
public class DestroyBlock {
    ...
    public DestroyBlock(Activity activity, ConstraintLayout classicGameLayout, Context context,
        ...
        myActivity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if(blockTouched == true) {  //block was touched
                    synchronized (GlobalVariables.class) {
                        for (GameBlock block : GlobalVariables.myGameBlocks) { //deletes all blocks on screen and awards points for every block
                            int[] location = block.getLocation();
                            if (location[1] <= screenLength) {
                                block.destroyBlockTouched = true;
                                block.handleBlockTouch();
                            }
                        }
                    }
                }
    ...
    public void handleBlockTouch(){ //handles the touch event accordingly
        ...
        if(ClassicActivity.classicPrefs.getBoolean(GlobalVariables.soundSwitch,true)) {
            ClassicActivity.sound.playBombSound();
        }
        myActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
